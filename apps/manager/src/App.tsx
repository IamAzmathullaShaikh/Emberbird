...
export const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100">
      <Header status={status} onRefresh={refreshStatus} loading={loadingStatus} />

      <main className="flex-1 overflow-y-auto p-6 max-w-5xl mx-auto w-full space-y-6">
        <StatusCard status={status} />
        <ReleaseCard updateStatus={updateStatus} onCheckUpdates={handleCheckUpdates} checking={checkingUpdates} />
        ...
        <div className="p-4 rounded-xl bg-slate-900/30 border border-slate-800/80 flex items-center justify-between text-xs text-slate-500">
          <span>WSABuilds Community Platform • Native Core Engine</span>
          <span>Sprint 6 Deliverable</span>
        </div>
      </main>
    </div>
  );
