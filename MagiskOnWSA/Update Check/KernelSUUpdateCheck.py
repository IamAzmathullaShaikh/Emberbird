...
url = f"https://raw.githubusercontent.com/{repo}/update/kernelsu.appversion"
...
if is_valid_version(currentver):
    with open('kernelsu.appversion', 'w', encoding='utf-8') as file:
...
try:
    ...
    latestver = str(json.loads(resp.content)['tag_name']).replace('v', '').strip().replace('\n', '')
except Exception as exc:
    print(f"Failed to fetch latest KernelSU version: {exc}")
    sys.exit(1)

if not is_valid_version(latestver):
    print(f"Invalid latest KernelSU version fetched: '{latestver}'")
    sys.exit(1)
...
if currentver != latestver:
    ...
    if not os.path.exists('kernelsu.appversion') or os.path.getsize('kernelsu.appversion') == 0:
        with open('kernelsu.appversion', 'w', encoding='utf-8') as file:
            file.write(latestver)
    kernelsumsg = f"KernelSU Version: {latestver}"

print(f"KernelSU Version status: {kernelsumsg}")
write_github_env("KERNEL_SU_MSG", kernelsumsg)
write_github_env("KERNEL_SU_VER", latestver)
