#!/usr/bin/python3
"""Check for a newer Windows Subsystem for Android (Retail) build via
Microsoft's FE3 delivery service and update the retail.appversion file.
...
url = f"https://raw.githubusercontent.com/{repo}/update/retail.appversion"
...
if not is_valid_version(currentver):
    ...
    try:
        with open('retail.appversion', 'w', encoding='utf-8') as file:
            file.write(currentver)
        print("retail.appversion file written.")
...
if wsa_build_ver in (None, 0):
    ...
    write_github_env("RETAIL_UPDATE", "no")
...
if current < latest:
    ...
    try:
        ...
        print("retail.appversion updated.")
    except Exception as e:
        print(f"Error writing to file: {e}")
    if currentver:
    ...
    write_github_env("LATEST_RETAIL_VER", str(wsa_build_ver))
    write_github_env("MSG", msg)
    write_github_env("RETAIL_UPDATE", "yes")
    write_github_env("WSA_UPDATE_STATUS", "AVAILABLE")
else:
    print(f"WSA Retail version is up to date: {currentver}")
    if not os.path.exists('retail.appversion') or os.path.getsize('retail.appversion') == 0:
        with open('retail.appversion', 'w', encoding='utf-8') as file:
            file.write(str(currentver if currentver else wsa_build_ver))
    write_github_env("WSA_UPDATE_STATUS", "NO_CHANGE")
    write_github_env("RETAIL_UPDATE", "no")
    write_github_env("SHOULD_BUILD", "no")
    write_github_env("LATEST_RETAIL_VER", str(currentver if currentver else wsa_build_ver))
